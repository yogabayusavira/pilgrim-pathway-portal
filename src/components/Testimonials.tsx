...
const Testimonials = () => {
  ...
  return <section id="testimonials" className="section-padding hajj-gradient text-white">
      <div className="container-custom">
        ...
        <div className="relative max-w-4xl mx-auto">
          <div className="overflow-hidden rounded-xl bg-white/10 backdrop-blur-sm p-6 md:p-10">
            {testimonials.map((testimonial, index) => <div key={testimonial.id} className={`transition-opacity duration-500 ${index === activeIndex ? 'opacity-100' : 'opacity-0 absolute top-0 left-0'}`} style={{
            display: index === activeIndex ? 'block' : 'none'
          }}>
                <div className="flex flex-col md:flex-row gap-6 items-start md:items-center mb-6">
                  <img src={testimonial.image} alt={testimonial.name} className="w-20 h-20 rounded-full border-4 border-gold-500" />
                  <div>
                    <h3 className="text-xl font-bold">{testimonial.name}</h3>
                    <p className="text-white/70">{testimonial.location}</p>
                    <div className="flex mt-1">
                      {[...Array(5)].map((_, i) => <Star key={i} size={16} className={i < testimonial.rating ? "fill-gold-500 text-gold-500" : "text-white/30"} />)}
                    </div>
            ...
            <div className="flex justify-between mt-8">
              <Button variant="outline" size="icon" onClick={handlePrev} className="border-white/30 hover:bg-white/10 hover:border-white text-slate-50">
                <ChevronLeft size={20} />
              </Button>
              
              <div className="flex gap-2">
                {testimonials.map((_, index) => <button key={index} onClick={() => setActiveIndex(index)} className={`w-3 h-3 rounded-full transition-colors ${index === activeIndex ? 'bg-gold-500' : 'bg-white/30 hover:bg-white/50'}`} aria-label={`Go to testimonial ${index + 1}`} />)}
              </div>
              
              <Button variant="outline" size="icon" onClick={handleNext} className="border-white/30 hover:bg-white/10 hover:border-white text-slate-50">
